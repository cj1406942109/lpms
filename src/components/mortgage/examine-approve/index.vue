  ...
  <div class="app-container">
    ...
    <div class="form-wrapper" v-if="activeStep=='0'">
      <h3>资料目录表</h3>
       <el-form :model="catalogForm" ref="catalogForm" label-width="200px" :rules="catalogFormRules" v-if="catalogForm" key="catalogForm">
        ...
        <el-form-item>
          <template v-if="finishCatalog">
            <el-button type="primary" @click="updateCatalogHandler()">更新</el-button>
            <el-button type="info" disabled>已确认收齐</el-button>
          </template>
          <el-button type="primary" @click="confirmCatalogHandler()" v-else>确认已收齐</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="form-wrapper" v-if="activeStep=='1'">
      ...
      <el-form :model="approveForm" ref="approveForm" label-width="200px" :rules="approveFormRules" key="approveForm">
        ...
        <el-form-item>
          <el-button type="info" disabled v-if="finishSend">已完成报审</el-button>
          <el-button type="primary" @click="completeApproveHandler()" v-else>完成报审</el-button>
        </el-form-item>
      </el-form>
    </div>
          ...
          <el-col :span="10">
            <template v-if="approveStatusForm.approveState == '1'">
              ...
              <el-form-item label="金额" prop="amount">
                <el-input clearable v-model.number="approveStatusForm.amount" type="number" min="0" :disabled="finishApprove"><template slot="append">元</template></el-input>
              </el-form-item>
              <el-form-item label="年限" prop="period">
                <el-input clearable v-model.number="approveStatusForm.period" type="number" min="0" :disabled="finishApprove"><template slot="append">年</template></el-input>
              </el-form-item>
              <el-form-item label="利率" prop="rate">
                <el-input clearable v-model.number="approveStatusForm.rate" type="number" min="0" :disabled="finishApprove"><template slot="append">%</template></el-input>
              </el-form-item>
            ...
            <template v-if="approveStatusForm.approveState == '0'">
              ...
              <el-form-item label="未通过原因" prop="failReason">
                ...
                <el-input clearable v-model="approveStatusForm.failReasonOther" placeholder="其他原因" v-if="approveStatusForm.failReason == '63'" :disabled="finishApprove"></el-input>
              </el-form-item>
    ...
    <div class="form-wrapper" v-if="activeStep=='3' && reportType == '1'">
      ...
      <el-form :model="reportForm" ref="reportForm" label-width="200px" :rules="reportFormRules" key="reportForm">
        ...
        <el-form-item v-if="!finishReport">
        ...
        <el-form-item v-else>
          <el-button type="info" disabled>已出正评</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="option">
  ...
  created () {
    ...
    getApproveById(this.approveId).then(({ data }) => {
      if (data) {
        ...
        this.finishApprove = data.confirmApproveState.done
        this.finishReport = data.approveComment.done
        this.catalogForm = JSON.parse(JSON.stringify(data.catalog))
        if (data.sendState.done) {
          this.approveForm.time = data.approve.time
        }
        if (data.confirmApproveState.done) {
        ...
        // 从返回的所有报告中，拆分出评估下单环节的报告，和审批环节的报告
        // 根据评估下单环节的报告类型做处理
        if (this.reportType === 1) {
          data.reports.forEach(house => {
            const tempHouse = []
            house.forEach(report => {
              // 报告中的所有预评暂存，保存到preReports
              if (parseInt(report.reportType) === 1) {
                tempHouse.push(JSON.parse(JSON.stringify(report)))
              } else {
                // 报告中的所有正评，直接保存到审批环节的reportForm
                this.reportForm.reports.push(JSON.parse(JSON.stringify(report)))
                this.reportForm.time = report.time
              }
            })
            // 筛选出的预评报告，保存到preReports中
            this.preReports.push(tempHouse)
          })
