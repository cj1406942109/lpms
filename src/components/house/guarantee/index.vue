    ...
    <div class="form-wrapper" v-if="activeStep=='0'">
      ...
      <el-form :model="guaranteeForm" ref="guaranteeForm" label-width="200px" :rules="guaranteeFormRules" key="guaranteeForm">
        <el-form-item label="是否需要盖章" prop="isNeedStamp">
          <el-radio-group v-model="guaranteeForm.isNeedStamp" :disabled="finishGuarantee">
            <el-radio :label="1">是</el-radio>
            <el-radio :label="0">否</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="盖章通过时间" prop="stampTime" v-if="guaranteeForm.isNeedStamp == '1'">
          <el-date-picker type="date" placeholder="选择日期" v-model="guaranteeForm.stampTime" value-format="timestamp" :disabled="finishGuarantee"></el-date-picker>
        </el-form-item>
        <el-form-item label="出担保函时间" prop="guaranteeTime">
          <el-date-picker type="date" placeholder="选择日期" v-model="guaranteeForm.guaranteeTime" value-format="timestamp" :disabled="finishGuarantee"></el-date-picker>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="formLoading" @click="saveGuaranteeHandler()" v-if="!finishGuarantee">提交</el-button>
          <el-button type="info" disabled v-else>已完成担保</el-button>
        </el-form-item>
      </el-form>
    </div>
  ...
  created () {
    ...
    getGuaranteeById(this.guaranteeId).then(({ data }) => {
      if (data) {
        ...
        if (this.finishGuarantee) {
          this.guaranteeForm.isNeedStamp = parseInt(data.guaranteeState.isNeedStamp)
          this.guaranteeForm.stampTime = data.guaranteeState.stampTime
          this.guaranteeForm.guaranteeTime = data.guaranteeState.guaranteeTime
        }
        this.preReport = JSON.parse(JSON.stringify(data.report))
        if (this.finishReport) {
          this.reportForm = JSON.parse(JSON.stringify(data.formalReport))
          this.reportForm.report.reportHouseAge = parseFloat(this.reportForm.report.reportHouseAge)
          this.reportForm.report.reportLoanYear = parseFloat(this.reportForm.report.reportLoanYear)
