    ...
    <el-form :model="checklistForm" ref="checklistForm" label-width="200px" :rules="rules">
      ...
      <el-row :gutter="20">
        <el-col :span="10">
          ...
          <el-form-item label="客户经理电话" prop="managerPhone">
...
import { getStaticIndexByKey, saveChecklist, createTask, getChecklistById, updateChecklist } from '@/api/house'
    ...
    if (this.checklistId) {
      getChecklistById(this.checklistId).then(({ data }) => {
        if (data) {
          ...
          // 类型转换
          this.checklistForm.loanPeriod = parseFloat(this.checklistForm.loanPeriod)
          this.checklistForm.houseArea = parseFloat(this.checklistForm.houseArea)
        } else {
          this.$message({
            type: 'error',
            message: '客户交接表数据获取失败'
