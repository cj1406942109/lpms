...
export default {
  ...
  methods: {
    confirmCheckHandler () {
      this.$refs['checkForm'].validate((valid) => {
        if (valid) {
          confirmCheck(this.$route.params.id, this.checkForm.houseTime, this.checkForm.creditTime).then(data => {
            if (data) {
    ...
    confirmIntegrateHandler () {
      this.$refs['catalogForm'].validate((valid) => {
        if (valid) {
          confirmIntegrate(this.$route.params.id, this.catalogForm).then(data => {
            if (data) {
      ...
      this.$refs['inputForm'].validate((valid) => {
        if (valid) {
          this.$confirm('请确认信息填写无误，是否提交？', '提示', {
            ...
            confirmInput(this.$route.params.id, this.inputForm.time).then(data => {
  ...
  created () {
    ...
    this.finishCheck = this.$route.params.checkState === 'true'
